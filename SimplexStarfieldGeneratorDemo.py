...
This version of the terrain generator library interpreter is set up to produce starfields using Simplex noise.

Important settings and inits specific to the Simplex generator can be found in the code by searching for cfref:simplex-gen
...
#### Simplex noise testing observations #### ## cfref:simplex-gen

## Frequency and octaves work in some kind of ratio to each other that would require an extensive review of the generator algorithm to fully understand the details of.

## Keep frequency low and octaves high.
#NOISE_FREQUENCY = 0.03

## Fewer octaves means faster calculation. Try to keep octaves fairly low and play with frequency.
#NOISE_OCTAVES = 8

## Higher frequency seems to mean rougher noise, lower seems to mean smoother noise.
#NOISE_PERSISTENCE = 0.5



## Starfieldish. Feel free to tweak all the settings you want. It's kind of fun, actually.

## Changing this number as little as +/- 0.1 points seems to have a huge effect on star density.
## Also, using 5.0 and 50.0 worked, but 30.0 and 40.0 had exaggerated bias... it's almost like changing the hash with the hash iterator (commented far below here, use grayscale if you're curious).
NOISE_FREQUENCY = 0.5

NOISE_OCTAVES = 2

## Persistence has to be a few zeroes away from 1, I think. 200 and 0.002 are both acceptable for starfields but 5 is way too sparse.
## This is fascinating to test but also very time consuming; I'm going to have to make a wrapper for this to flip them around on the fly!
NOISE_PERSISTENCE = 200

## Remember to keep star color values of <= 3, <= 6, <= 9 instead of == 1, == 2, == 3.
## It makes it easier to find positives, but is it actually the best bet?
## It could be that there's a decent set of parameters that creates star-like spikes at z values of 1, 2 and 3, but then again maybe not, due to the way simplex noise is generated.




## (( earlier testing notes are below ))
...
while keep_window_open == True:
    ...
    ## If using a simplex noise generator: ## cfref:simplex-gen
