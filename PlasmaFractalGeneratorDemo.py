...
This version of the terrain generator library interpreter is set up to produce terrain maps using noise from a plasma fractal.

Important settings and inits specific to the plasma fractal generator can be found in the code by searching for cfref:plasma-gen
...
## I wrote this before I realized I wanted to provide sensible silent defaults for all the little options.
## The following lines aren't necessary but remain as examples of how to change the resulting fractal and if uncommented will create a specific flavor of map every time the generator starts.
## Displacement min/max changes how textured the terrain is (and whether it has a positive or negative bias, if they're out of balance). These both have defaults in the generator's init.
## Reinitializing the corners will create new landforms/plasma cloudbanks.
...
while keep_window_open == True:
    ...
    ## If using a plasma fractal generator: cfref:plasma-gen
    the_plasma_generator.reinitialize_corners() # This line randomizes the corners. Comment it to see variations on the same sort of terrain map (which is in that case decided at generator initialization). Accepts parameters if you want specific corner heights, for example: reinitialize_corners(uleft_corner=135, uright_corner=245, lleft_corner=135, lright_corner=135)
